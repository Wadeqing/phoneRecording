...
import com.sinocall.phonerecordera.ui.activity.DownLoadRecoedActivity;
...
public class DownLoadRecordingRecyclerViewAdapter extends RecyclerView.Adapter<DownLoadRecordingRecyclerViewAdapter.ViewHolder> {
    private BaseActivity mContext;
    private List<FileDownloadBean> list = new ArrayList<>();

    public DownLoadRecordingRecyclerViewAdapter(List<FileDownloadBean> fileList, DownLoadRecoedActivity downLoadRecoedActivity) {
        this.list = fileList;
        this.mContext = downLoadRecoedActivity;
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        if (list != null && list.size() > 0) {
            FileDownloadBean dataBean = list.get(position);
            if (dataBean != null && holder != null) {
                holder.textviewRecordName.setText(dataBean.getFileName());
                holder.textviewRecordTime.setText(dataBean.getCreateTime());
                holder.textviewRecordSize.setText(FileSizeUtil.getPrintSize(Integer.parseInt(dataBean.getFileSize() + "")));
            }
        }
        // 如果设置了回调，则设置点击事件
